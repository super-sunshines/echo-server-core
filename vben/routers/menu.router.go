...
var MenuRouterGroup = core.NewRouterGroup("/system/menu", NewMenuRouter, func(rg *echo.Group, group *core.RouterGroup) error {
	return group.Reg(func(m *MenuRouter) {
		...
		rg.DELETE("", m.deleteMenu, core.HavePermission("SYS::MENU::DEL"), core.Log("删除目录"))
		rg.GET("/new/codes", m.newCodeList, core.IgnorePermission(), core.Log("新增的权限代码"))
		rg.POST("/code", m.addCode, core.HavePermission("SYS::MENU::CODE::ADD"), core.Log("新增权限代码"))
		rg.PUT("/code/:id", m.editCode, core.HavePermission("SYS::MENU::UPDATE"), core.Log("修改权限代码"))
		rg.GET("/name-exists", m.nameExist, core.HavePermission("SYS::MENU::QUERY"), core.Log("名称是否存在"))
		rg.GET("/path-exists", m.pathExist, core.HavePermission("SYS::MENU::QUERY"), core.Log("路径是否存在"))
	})
})
